...
def hash_login_key(key: str) -> str:
    ...
    return hmac.new(
        pepper.encode("utf-8"), norm.encode("utf-8"), hashlib.sha256
    ).hexdigest()
...
class Database:
        ...
        async with self._pool.acquire() as conn:
            ...
            try:
                ...
                if is_pk > 0:
                    ...
                    if pk_name:
                        await conn.execute(
                            f"ALTER TABLE users DROP CONSTRAINT {pk_name} CASCADE"
                        )
                ...
                await conn.execute(
                    "CREATE UNIQUE INDEX IF NOT EXISTS idx_users_login_key_hash ON users(login_key_hash)"
                )
                ...
                try:
                    await conn.execute(
                        "ALTER TABLE bids ALTER COLUMN manager DROP NOT NULL"
                    )
                except:
                    pass
                ...
                # 1. Get all columns
                cols = await conn.fetch(
                    "SELECT column_name, is_nullable FROM information_schema.columns WHERE table_name = 'comments' AND table_schema = 'public'"
                )
                ...
                for c in cols:
                    if c["column_name"] not in ("id", "request_id"):
                        await conn.execute(
                            f'ALTER TABLE comments ALTER COLUMN "{c["column_name"]}" DROP NOT NULL'
                        )
                ...
                if "text" not in col_names:
                    if "comment" in col_names:
                        await conn.execute(
                            "ALTER TABLE comments RENAME COLUMN comment TO text"
                        )
                    else:
                ...
                if "type" not in col_names:
                    await conn.execute(
                        "ALTER TABLE comments ADD COLUMN type TEXT DEFAULT 'user'"
                    )
                ...
                try:
                    await conn.execute(
                        "CREATE UNIQUE INDEX IF NOT EXISTS idx_users_login_key ON users(login_key)"
                    )
                except Exception as e:
                ...
                for col in [
                    ...
                    if col not in req_cols:
                        await conn.execute(
                            f"ALTER TABLE requests ADD COLUMN IF NOT EXISTS {col} TEXT"
                        )

                if "mute_reminders" not in req_cols:
                    await conn.execute(
                        "ALTER TABLE requests ADD COLUMN IF NOT EXISTS mute_reminders BOOLEAN DEFAULT FALSE"
                    )
                ...
                if "manager_name" not in bid_cols:
                    await conn.execute(
                        "ALTER TABLE bids ADD COLUMN IF NOT EXISTS manager_name TEXT"
                    )
                if "status" not in bid_cols:
                    await conn.execute(
                        "ALTER TABLE bids ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'Ожидание'"
                    )

                # Add unique index for upsert
                await conn.execute(
                    "CREATE UNIQUE INDEX IF NOT EXISTS idx_bids_request_user ON bids(request_id, user_id)"
                )

                # Add missing bid columns
                if "loading_hours" not in bid_cols:
                    await conn.execute(
                        "ALTER TABLE bids ADD COLUMN IF NOT EXISTS loading_hours TEXT"
                    )
                if "demurrage" not in bid_cols:
                    await conn.execute(
                        "ALTER TABLE bids ADD COLUMN IF NOT EXISTS demurrage TEXT"
                    )
    ...
    async def list_bids(self, request_id: int):
        async with self._pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT * FROM bids WHERE request_id = $1 ORDER BY created_at DESC",
                request_id,
            )
            return [dict(r) for r in rows]
    ...
    async def find_user_by_key(self, login_key: str):
        ...
        async with self._pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT * FROM users WHERE login_key_hash = $1", key_hash
            )
            return dict(row) if row else None
    ...
    async def save_user(
        self, telegram_id: int, name: str, role: str, login_key: str = None
    ):
    ...
    async def list_requests(
        ...
        **kwargs,
        ...
        if manager:
            ...
            query += (
                f" AND (responsible = ${len(params)} OR creator_name = ${len(params)})"
            )
        if search:
    ...
    async def upsert_bid(
        self, request_id: int, user_id: int, manager_name: str, fields: dict
    ):
        async with self._pool.acquire() as conn:
            await conn.execute(
                """
                ...
                fields.get("currency"),
                fields.get("validity"),
                fields.get("payment_terms") or fields.get("payment_method"),
                fields.get("loading_hours"),
                fields.get("demurrage"),
                fields.get("comment"),
            )
    ...
    async def get_user_bid(self, request_id: int, user_id: int):
        async with self._pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT * FROM bids WHERE request_id = $1 AND user_id = $2",
                request_id,
                user_id,
            )
            return dict(row) if row else None
    ...
    async def add_comment(
        self, request_id: int, user_id: int, user_name: str, text: str, type="user"
    ):
        async with self._pool.acquire() as conn:
            await conn.execute(
    ...
    async def add_attachment(
        self, request_id, file_name, file_path, file_type, file_size
    ):
        async with self._pool.acquire() as conn:
            row = await conn.fetchrow(
    ...
    async def add_tariff(
        self, title, file_name, file_path, file_type, file_size, uploader_name
    ):
        async with self._pool.acquire() as conn:
            await conn.execute(
    ...
    async def get_stats(self, days=30):
        ...
        def apply_filter(q):
            if "WHERE" in q.upper():
                return q.replace(
                    "WHERE",
                    f"WHERE {date_filter.replace('$1', '1') if params else '1=1'} AND",
                )
            else:
                return (
                    q + f" WHERE {date_filter.replace('$1', '1') if params else '1=1'}"
                )
        ...
        async with self._pool.acquire() as conn:
            ...
            # 3. Success Rate
            date_filter = (
                f"AND created_at >= NOW() - INTERVAL '{days} days'" if days > 0 else ""
            )
            ...
            """)
            success_rate = (
                round((successful / total_closed * 100), 1) if total_closed else 0.0
            )
    ...
    async def rotate_user_key(
        self, user_id: int = None, old_key: str = None, new_key: str = None
    ):
    ...
    async def update_user_profile(
        self, user_id: int, name: str = None, role: str = None
    ):
        ...
        async with self._pool.acquire() as conn:
            await conn.execute(
                f"UPDATE users SET {', '.join(sets)} WHERE id = ${len(vals)}", *vals
            )
    ...
    async def get_ai_context(self, user_id: int):
        async with self._pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT draft, history FROM ai_sessions WHERE user_id = $1", user_id
            )
            if row:
