                ...
                for r in reqs_to_poll:
                    ...
                    if creator_id:
                        ...
                        keyboard = [
                            [
                                InlineKeyboardButton(
                                    "✅ Подтвердили",
                                    callback_data=f"fb_{req_id}_confirmed",
                                )
                            ],
                            [
                                InlineKeyboardButton(
                                    "❌ Клиент не заинтересован",
                                    callback_data=f"fb_{req_id}_not_interested",
                                )
                            ],
                            [
                                InlineKeyboardButton(
                                    "⏳ Ждём до даты",
                                    callback_data=f"fb_{req_id}_wait_date",
                                )
                            ],
                            [
                                InlineKeyboardButton(
                                    "🔄 Нужна другая ставка",
                                    callback_data=f"fb_{req_id}_need_rate",
                                )
                            ],
                            [
                                InlineKeyboardButton(
                                    "📝 Другое", callback_data=f"fb_{req_id}_other"
                                )
                            ],
                        ]
                        ...
                        try:
                            ...
                            logger.error(
                                f"Failed to send feedback poll to {creator_id} for req {req_id}: {e}"
                            )
                    ...
                    try:
                        ...
                        keyboard = [
                            [
                                InlineKeyboardButton(
                                    "🔔 Напомнить логистам",
                                    callback_data=f"ping_logistics_conf_{req['id']}",
                                )
                            ],
                            [
                                InlineKeyboardButton(
                                    "💤 Напомнить позже",
                                    callback_data=f"remind_later_{req['id']}",
                                )
                            ],
                            [
                                InlineKeyboardButton(
                                    "🔇 Отключить",
                                    callback_data=f"remind_mute_{req['id']}",
                                )
                            ],
                        ]
                        await bot.send_message(
                        ...
                        await db.update_request(
                            req["id"], {"last_notified_at": datetime.now(TZ)}
                        )
                    except Exception as e:
